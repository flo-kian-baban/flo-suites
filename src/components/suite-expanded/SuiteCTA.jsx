...
const SuiteCTA = ({ suiteName, suiteId }) => {
    const isStudio = suiteId === 'studio';
    ...
    return (
        <div className="p-6 border-t border-white/20 bg-black/50 backdrop-blur-3xl z-20 shrink-0 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)]">
            <div className={`max-w-6xl mx-auto flex items-center ${isStudio ? 'justify-center' : 'flex-col md:flex-row justify-between'} gap-6`}>
                {/* Helper text — hidden for Studio */}
                {!isStudio && (
                ...
                <div className={`flex items-center gap-4 ${isStudio ? 'w-auto' : 'w-full md:w-auto'}`}>
                    {/* Explore Flo OS — hidden for Studio */}
                    {!isStudio && (
